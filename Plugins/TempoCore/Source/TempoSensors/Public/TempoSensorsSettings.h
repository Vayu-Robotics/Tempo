...
private:
	// Whether to block the game thread on camera rendering. This can slightly decrease the latency from camera trigger
	...
	// The max number of camera renders to buffer before dropping frames.
	UPROPERTY(EditAnywhere, Config, Category="Camera", AdvancedDisplay)
